...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/18 19:01:44 by anremiki          #+#    #+#             */
/*   Updated: 2022/04/18 19:01:45 by anremiki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	doors(t_mlx *ptr, int x, int y)
{
	char **map;

	map = ptr->map;
	if (y > 0 && (map[y - 1][x] == 'd' || map[y - 1][x] == 'D'))
		return (swap_doors(map, x, y - 1));
	if (y + 1< ptr->my && (map[y + 1][x] == 'd' || map[y - 1][x] == 'd'))
		return (swap_doors(map, x, y + 1));
	if (x > 0 && (map[y][x - 1] == 'd' || map[y][x -1] == 'D'))
		return (swap_doors(map, x - 1, y));
	if (x + 1 < ptr->mx && (map[y][x + 1] == 'd' || map[y][x + 1] == 'D'))
		return (swap_doors(map, x + 1, y));
	return (0);
}
...
int	release(int keycode, t_mlx *ptr)	//permet le double input
{
	...
	if (keycode == 'f')
	{
		dir = doors(ptr, ptr->px / 64, ptr->py / 64);
		if (dir != 0)
		{
			printf("angle = %f\n", ptr->pa);
			free(ptr->exp);
